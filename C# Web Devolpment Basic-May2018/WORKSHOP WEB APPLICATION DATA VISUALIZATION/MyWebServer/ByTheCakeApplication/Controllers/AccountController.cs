namespace MyWebServer.ByTheCakeApplication.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public IHttpResponse Register(IHttpRequest req, RegisterUserViewModel model)
        {
            ...
            if ((model.Name.Length < 3 || string.IsNullOrEmpty(model.Name))
                || (model.Username.Length < 3 || string.IsNullOrEmpty(model.Username))
                || (model.Password.Length < 3 || string.IsNullOrEmpty(model.Password))
                || model.ConfirmPassword != model.Password)
            {
             ...
             var success = this.users.Create(model.Name, model.Username, model.Password);
